...
function buildPage(sessionDays: number): string {
  return `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Connect to Hermes</title><style>body{font:16px system-ui,-apple-system,sans-serif;background:#090a0f;color:#f3f4f6;margin:0;padding:24px}main{max-width:420px;margin:8vh auto;background:#13141d;border:1px solid rgba(245,197,66,0.35);padding:28px;border-radius:22px;box-shadow:0 8px 32px rgba(0,0,0,0.6)}h1{font-size:25px;color:#f5c542;margin-top:0}input,button{box-sizing:border-box;width:100%;padding:14px;margin:8px 0;border-radius:12px;border:1px solid #232533;background:#191b26;color:#fff;font:inherit}button{background:#f5c542;color:#090a0f;font-weight:600;border:none;cursor:pointer}button:active{transform:scale(0.98)}p{line-height:1.6;color:#8b8f9e}a{color:#f5c542}#qr svg{width:100%;height:auto;border-radius:12px;background:#fff;padding:8px}li{margin:16px 0;overflow-wrap:anywhere}#status{color:#f87171}button:disabled{opacity:.5}#pair-link{word-break:break-all;color:#f5c542;text-decoration:underline;margin:12px 0;display:block}</style><main><h1>✦ Connect to Hermes</h1><p id="intro">Enter the access password, or pair with a QR code generated on your computer.</p><form id="login"><input id="password" type="password" autocomplete="current-password" placeholder="Access Password" required><button>Sign In & Remember Device</button></form><section id="manage" hidden><p>Scan the code or open the link on your phone. Code is valid for 5 minutes (single-use); session persists for ${sessionDays} days.</p><button id="generate">Generate Pairing QR Code</button><div id="qr"></div><div id="pair-container"></div><p id="expiry"></p><h2>Connected Devices</h2><ul id="devices"></ul><button id="logout">Sign Out</button><p><a href="/">Return to Hermes →</a></p></section><p id="status" role="status"></p></main><script type="module" src="/auth/client.js"></script></html>`
}

const client = `const $=id=>document.getElementById(id);const status=$('status');async function api(path,data){const r=await fetch('/auth/'+path,{method:data?'POST':'GET',headers:data?{'Content-Type':'application/json'}:{},body:data?JSON.stringify(data):undefined});const result=await r.json();if(!r.ok)throw Error(result.error||'Connection failed, please retry');return result}async function devices(){const d=await api('devices');$('devices').replaceChildren();for(const s of d.sessions){const li=document.createElement('li');li.textContent=s.name+(s.current?' (Current device)':'')+' · '+new Date(s.created).toLocaleDateString();const b=document.createElement('button');b.textContent='Disconnect';b.onclick=()=>run(async()=>{await api('revoke',{id:s.id});if(s.current)location.reload();else await devices()});li.append(b);$('devices').append(li)}}async function run(fn){status.textContent='';try{await fn()}catch(e){status.textContent=e.message;if($("manage").hidden)$("login").hidden=false}}$('login').onsubmit=e=>{e.preventDefault();run(async()=>{await api('login',{password:$('password').value});location.replace('/')})};let timer;$('generate').onclick=()=>run(async()=>{const b=$('generate');b.disabled=true;try{const d=await api('pair',{});$('qr').innerHTML=d.svg;if(d.link){const container=$('pair-container');container.replaceChildren();const a=document.createElement('a');a.id='pair-link';a.href=d.link;a.target='_blank';a.textContent=(d.isLan?'[LAN Link] ':'[Public Link] ')+d.link;container.append(a)}clearInterval(timer);timer=setInterval(()=>{const left=Math.max(0,Math.ceil((d.expires-Date.now())/1000));$('expiry').textContent=left?'Expires in '+left+'s':'QR code expired, please generate again';if(!left){$('qr').replaceChildren();if($('pair-container'))$('pair-container').replaceChildren();clearInterval(timer)}},1000)}finally{b.disabled=false}});$('logout').onclick=()=>run(async()=>{await api('logout',{});location.replace('/')});run(async()=>{const token=new URLSearchParams(location.hash.slice(1)).get('pair');if(token){history.replaceState(null,'','/connect');$('login').hidden=true;status.textContent='Connecting…';await api('redeem',{token});location.replace('/');return}const d=await api('status');if(d.authenticated){$('login').hidden=true;$('manage').hidden=false;$('intro').textContent='Connect your phone or manage signed-in devices here.';await devices()}});`
