...
export function accessPlugin(configuredPassword?: string, publicUrl?: string, storageDirectory = join(homedir(), '.hermes-web-shell')): Plugin {
  ...
  const middleware = async (req: IncomingMessage, res: ServerResponse, next: () => void) => {
    ...
    const issue = () => {
      ...
      sessions[hash(token)] = { id: randomBytes(12).toString('hex'), expires: Date.now() + 30 * 86400000, created: Date.now(), name: /iPhone|iPad/.test(ua) ? 'iPhone / iPad' : /Android/.test(ua) ? 'Android' : '电脑浏览器' }
      save(); cookie(token, 30 * 86400)
    }
    try {
      ...
      if (req.method === 'POST' && path.startsWith('/auth/')) {
        ...
        if (path === '/auth/pair') {
          ...
          if (url.protocol !== 'https:' || url.username || url.password) { json(400, { error: '请先配置手机可访问的 HTTPS 域名（SHELL_PUBLIC_URL）' }); return }
          ...
          json(200, { svg: await QRCode.toString(link, { type: 'svg', margin: 2 }), ...(localManager ? { png: await QRCode.toDataURL(link, { width: 320, margin: 2 }) } : {}), expires }); return
        }
